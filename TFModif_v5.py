...
# In[5]:
def get_velocity_loss(outputs, targets, fk_routine, leg_loss_coeff=7, ftct_loss_coeff=0.008): #will be defined as movement of poses from time1-time0, time2-time1, time3-time2 etc.
...
def train(train_dataloader, s2ag_generator, s2ag_discriminator, fk_routine, start_time, device):
    ...
    for epoch in range(s2ag_start_epoch, s2ag_num_epochs):
        ...
        # training
        print('DanceAnyWay training epoch: {:>4d}/{:d}, tf_ratio {}'.format(epoch + 1, s2ag_num_epochs,tf_ratio))
        ...
        if not epoch % save_interval:
            torch.save(s2ag_generator.state_dict(), './BN_RN_TFModif_v5/M2AD_Gen11_ep'+str(epoch)+'_10FPS')
            torch.save(s2ag_discriminator.state_dict(), './BN_RN_TFModif_v5/M2AD_Disc11_ep'+str(epoch)+'_10FPS')
