...
#         self.embed = nn.Embedding(label_classes,self.embed_vector_length)
    ...
    def forward(self, pre_seq, in_mfcc, in_chroma, label_val):
        ...
        encoder_states, (hidden,cell) = self.lstm(in_data)

        hidden = self.fc_hidden(torch.cat((hidden[0:5], hidden[5:10]), dim=2))
        cell = self.fc_cell(torch.cat((cell[0:5], cell[5:10]), dim=2))
...
class PoseDecoder(nn.Module):
    def __init__(self, args):
        ...
        self.hidden_flattener = nn.Sequential(
    ...
    def forward(self, in_data, encoder_states, hidden, cell):
        ...
        hidden_permuted = torch.cat((hidden_permuted,hidden[3:4]),dim=2)
        hidden_permuted = torch.cat((hidden_permuted,hidden[4:5]),dim=2)
