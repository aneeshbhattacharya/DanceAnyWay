...
#what if pose input is of shape batch, 3, 52
...
class Seq2SeqGenerator(nn.Module):
    ...
    def forward(self, pre_seq, in_mfcc, in_chroma, label_val, target, teacher_force_ratio):
        ...
        for t in range(1, target.shape[1]):
            output, hidden, cell = self.decoder(input, hidden*0.7 + hidden_original *0.3, cell)

            outputs[:,t,:] = output.squeeze(1) 

            # if t<3:
            #     input = target[:,t,:].unsqueeze(1) if random.random() < 0.8 else output #utilizes the 1st 3 input beats with more probability
